...
const LONG_CONTENT = Array.from({ length: 50 }, (_, i) => `Item ${i + 1}`);
  ...
  return (
    <div style={{ fontFamily: "sans-serif", padding: 32 }}>
      ...
      <div style={{ display: "flex", gap: 24, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
          Thumb hover:
          <input type="color" value={thumbHoverColor} onChange={(e) => setThumbHoverColor(e.target.value)} />
        </label>
      ...
      <div style={{ height: 300, width: 300, border: "1px solid #ccc", ...cssVars }}>
        <SleekScrollbar side={side} thumbMinHeight={thumbMinHeight}>
          <div>
            {LONG_CONTENT.map((item) => (
              <div key={item} style={{ padding: "8px 16px", borderBottom: "1px solid #eee" }}>
                {item}
              </div>
            ))}
          </div>
